...
4. **彩度规划**：以输入色彩度为基准（可用 `chromaBoost` 助推），按 10 级分段系数缩放（中级饱满、两端收敛防过曝），再按 **Cmax(L, H) 色域包络**（二分求解 sRGB 边界）规划每级彩度预算——在设计阶段避开色域悬崖，而非事后硬 clamp。浅级系数刻意不压得过低：淡而不着色的「灰白」正是浅色发飘的成因，此处要的是有明确色相的淡色。
...
- **Helmholtz–Kohlrausch 效应**（高彩度色在同等物理明度下看起来更亮）→ 中级明度按色相权重下压约 2%，恢复感知步进均匀。补偿以**绝对彩度**归一（C=0.15 达满强度）而非「彩度占色域包络比例」：后者在最浅/最深级会因 Cmax 本身极窄而把比例放大到接近 1，导致低彩度浅级被过度压暗、与页面底贴死
