...
var deltaE = differenceCiede2000();
...
function ensurePerceptibleSteps(levels, dark) {
  ...
  for (let i = 1; i < out.length; i += 1) {
    ...
    while (deltaE(out[i - 1], out[i]) < MIN_DE && guard < 10) {
      ...
      const clamped = clampChroma(out[i], "oklch");
      out[i].c = clamped.c;
      guard += 1;
    }
  }
...
function locatePrimary(input, colors) {
  ...
  colors.forEach((hex, i) => {
    const d = deltaE(input, parse(hex));
    if (d < bestDist) {
      bestDist = d;
      best = i;
    }
...
function generateNeutral(input) {
  ...
  if (hit) return hit;
  const source = toOklch2(parse(input));
...
function harmonyColors(input) {
  const base = resolveBase(input);
  const at = (h) => {
    if (base.neutral) return formatHex({ mode: "oklch", l: base.l, c: 0, h: 0 }) ?? "#808080";
    const clamped = clampChroma({ mode: "oklch", l: base.l, c: base.c, h: h % 360 }, "oklch");
...
function apcaContrast(text, bg) {
  return calcAPCA(toChannels(text), toChannels(bg));
}
function bestTextOn(bg) {
  const score = (fg) => Math.min(wcagContrast(bg, fg) / 4.5, Math.abs(apcaContrast(fg, bg)) / 60);
